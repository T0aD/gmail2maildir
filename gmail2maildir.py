...
# Getting first [Gmail]/All Mail

uid = re.compile(r'\(UIDVALIDITY (\d+)\)$')
for folder in folders:
    if folder in skip:
        print 'skipping', folder
        continue

#    if folder != 'INBOX' and folder != '[Gmail]/All Mail':
#        continue
...
#    print('=' * 10, folder)
    ...
    if typ != 'OK':
        print(typ, data)
        continue
#        raise Exception("Couldnt get UIDVALIDITY FOR %s" % folder)
#    print(data[0])
    ...
    typ, num = imap.select('"%s"' % folder, readonly=True)
    num = int(num[0])
    if typ != 'OK':
        raise Exception("Couldnt select folder %s" % folder)
    print "%20s" % folder, "%12d" % uidvalidity, "%8d" % num

    continue

#    uv, uidv = imap.response('UIDVALIDITY')
#    print(uv, int(uidv[0]))

#    typ, data = imap.fetch('1:4', '(UID)')
#    typ, data = imap.fetch('13014:*', '(UID)')
#    typ, data = imap.search(None, '(UID 158247:*)')
    typ, data = imap.search(None, '(UID 354858:*)')
#    print data[0]
#    continue
    data = data[0].split()
#    print data
...
#        id, g, g = msg.split()
        id = msg
        typ, data = imap.fetch(id, '(UID X-GM-MSGID BODY.PEEK[HEADER.FIELDS (SUBJECT)])')
#        typ, data = imap.fetch(id, '(UID RFC822)')
        print data[0]
        first = False
#    print data
    print 'skipped', count - 1, 'messages'
    continue


#    typ, data = imap.search(None, 'ALL')
    typ, data = imap.search(None, '(UID 35689:*)')
#    print(typ)
#    print(data)
#    print(data[0:200])
    ...
    l = data[0].split()
    for msg in l:
        typ, data = imap.fetch(msg, '(UID)')
        print 'id', msg, 'uid', data[0]

    continue
    typ, data = imap.fetch(num, '(UID RFC822.SIZE)')
    print(typ, data)
    continue

    typ, data = imap.fetch(num, '(BODY.PEEK[HEADER.FIELDS (MESSAGE-ID DATE FROM SUBJECT)])')
#    print(typ, data)
    print(typ, data[0][1])    
